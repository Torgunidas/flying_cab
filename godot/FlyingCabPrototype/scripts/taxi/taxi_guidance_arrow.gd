...
func _process(_dt: float) -> void:
	...
	var position := cab.get_global_transform_interpolated().origin
	var delta := target.landing_point() - position
	delta.z = 0
	if delta.length_squared() < 0.0001 or camera.is_position_behind(position):
		hide()
		return
	var cab_screen := camera.unproject_position(position)
	...
	direction = (camera.unproject_position(position + delta.normalized()) - cab_screen).normalized()
