...
func _ready() -> void:
	...
	# Dodaj do grupy vehicles (dla playera)
...
func _physics_process(delta: float) -> void:
	...
	if is_on_floor():
		if Input.is_action_just_pressed("thrust_up") or Input.is_action_just_pressed("thrust_right") or Input.is_action_just_pressed("thrust_left"):
			_is_boosting = true
			_boost_timer = 0.0
	else:
		_is_boosting = false
...
# zwykła siła
	...
	if _is_boosting:
		current_thrust *= ground_boost_vertical_multiplier
		velocity.y -= current_thrust * delta
