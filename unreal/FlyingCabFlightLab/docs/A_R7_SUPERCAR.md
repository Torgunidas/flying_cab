...
Verified on macOS (Apple M5, UE 5.8, Xcode 26.1.1) on 2026-09-10 during the project audit (`docs/AUDYT_PROJEKTU_2026-09-10.md` at the repository root): the Editor Development build succeeded without warnings, and `FlyingCab.Functional.PIE.Supercar` passed in two full NullRHI runs of the suite (46 tests each, 45/46; the single failure in each run was an unrelated intermittent test: `InputSoak.Seed1977` on a cold first PIE session, then a `MetroTrafficFlow` traffic stall). The macOS module had to be rebuilt after pulling; a stale `Binaries/Mac` dylib loads without any prompt and shows the pre-supercar world. A manual driving session on macOS has still not been performed. Since 2026-09-10 the world bootstrap logs `A_R7 supercars parked: 4/4.`; a PIE log without that line comes from a module built before this change.
