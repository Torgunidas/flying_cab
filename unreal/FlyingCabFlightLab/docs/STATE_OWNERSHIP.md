...
| Mapowanie klawiatury | `Content/Input/IMC_FlyingCabGameplay` i siedem `IA_FlyingCab*`; kontekst instaluje `AFlyingCabPlayerController` | utworzenie lokalnego gracza; `Completed`/`Canceled` zeruje ciągłe osie, a `FlushPressedKeys` synchronicznie czyści stan sterowanego obiektu | sesja lokalnego gracza; pojazd i postać piesza bindują ten sam zestaw akcji Enhanced Input, bez legacy mappings |
...
| Tożsamość, tematy NPC i definicje rozmów | `UFlyingCabNpcDefinition`, `UFlyingCabDialogueDefinition`; lista `UFlyingCabNpcRoster` lub aktor na mapie określa lokalizację | edycja assetów; walidacja w edytorze | konfiguracja; runtime nie modyfikuje treści |
| Bieżąca rozmowa i dostępne odpowiedzi | `UFlyingCabDialogueSession` na `AFlyingCabPlayerController` | zamknięcie, zmiana trybu/possession, utrata NPC lub poziomu | jedna rozmowa; widget prezentuje widok z numerem rewizji, akcje wywołują publiczne API questów |
| Postęp animacji rozmowy | `UFlyingCabDialogueWidget`: `FFlyingCabTypewriter` i `FFlyingCabDialogueStage` z `FlyingCabDialoguePresentation.h` | nowa kwestia z sesji, pominięcie przez gracza, zamknięcie okna | wyłącznie prezentacja; liczona zegarem ściennym, bo rozmowa pauzuje świat, i nie wpływa na stan questów. Szczegóły: `NPC_CONVERSATION_UI.md` |
| Podgląd rozmowy w edytorze | osobna sesja `UFlyingCabDialogueSession` w trybie preview | restart podglądu lub zamknięcie okna | symulacja; nie posiada subsystemu gry i nie przyznaje nagród |
| Projekcja dziennika zadań | `UFlyingCabQuestSubsystem::GetJournalEntries`; widok `UFlyingCabQuestJournalWidget` nie posiada stanu domeny | zmiana stanu zadania, wybór `TRACK`/`STOP TRACKING`, przełączenie zakładki UI | dziennik pokazuje tylko zadania przyjęte; zamknięcie ekranu zwalnia wejścia i wznawia grę |
| Komunikaty graczowe | `UFlyingCabTouchControls`: osobna kolejka małych wiadomości i priorytetowa kolejka dużych plansz | presenter HUD oraz `FFlyingCabQuestUpdate`; animacja jest wyłącznie prezentacją | wiadomości nie zmieniają stanu gameplayu i nie mogą zastępować się przed końcem czasu ekspozycji |
| Najlepsze czasy Time Attack | `UFlyingCabScoreSaveGame` | brak automatycznego resetu | dysk; format oznaczony `SaveVersion` |
| Układ aktywnej mapy: przystanki, stacje i trasy | `FlightLab.umap`: `AFlyingCabDistrictAnchor`, aktory usług i `AFlyingCabLivingRoute`; odczyt przez funkcje `GetWorld*` w `FlyingCabCityData` | edycja i zapis mapy; kopia dla PIE | mapa jest źródłem położenia kursów i znaczników usług; asset `DA_FlyingCabCityLayout` zachowuje domyślne definicje, granice minimapy i obszary autostrad. Szczegóły: `WORLD_EDITING.md` |
| Stała infrastruktura i uruchamianie świata | geometria, usługi, biuro, portale, terminal i NPC zapisane w mapie; referencje `AFlyingCabAuthoredWorld` | wczytanie poziomu | bootstrap rejestruje zapisane obiekty; tworzy pojazdy z zapisanych punktów oraz populację na zapisanych trasach. Nie regeneruje aktywnej mapy. |
