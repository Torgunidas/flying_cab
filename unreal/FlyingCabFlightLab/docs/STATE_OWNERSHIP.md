...
| Mapowanie klawiatury | `Content/Input/IMC_FlyingCabGameplay` i sześć `IA_FlyingCab*`; kontekst instaluje `AFlyingCabPlayerController` | utworzenie lokalnego gracza; `Completed`/`Canceled` zeruje ciągłe osie | sesja lokalnego gracza; pojazd i postać piesza bindują ten sam zestaw akcji Enhanced Input, bez legacy mappings |
